...
public class TradeService {
    ...
    public TradeResponseDTO createTradeRequest(TradeRequestDTO request) {
        log.info("Creating trade request for listing: {} by user: {}", request.getListingId(), request.getRequestingUserId());
        ...
        tradeRepository.findPendingTradeByListingAndUser(request.getListingId(), request.getRequestingUserId())
                .ifPresent(existing -> {
        ...
        publishTradeEvent("TRADE_CREATED", savedTrade.getTradeId(), request.getListingId(), request.getRequestingUserId());
    ...
    public TradeResponseDTO acceptTradeRequest(Long tradeId, Long listingOwnerId) {
        ...
        Trade trade = tradeRepository.findByIdWithOfferedCards(tradeId)
        ...
        try {
            listingServiceClient.updateListingStatus(trade.getListingId(), 
                    new ListingServiceClient.ListingStatusUpdate("complete"));
        } catch (Exception e) {
            log.error("Failed to update listing status", e);
        }
        
        // Reject all other pending trades for this listing
        List<Trade> otherPendingTrades = tradeRepository.findByListingIdAndTradeStatus(trade.getListingId(), TradeStatus.pending);
        otherPendingTrades.stream()
